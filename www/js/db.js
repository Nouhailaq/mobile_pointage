function odoo_project_pointing_db(project_pointing) {

    //Store data in localstorage, { projects: [records(i.e. project_id, task_id, date, hours, descriptions etc)] }
    //When project_pointing_model's load_stored_data is called it will create project model and add it into project collection, one project model even though same project is used twice or thrice in records
    project_pointing.project_pointing_db = openerp.Class.extend({
        ...
        add_project_pointing_session: function(project_pointing_session) {
            this.save("project_pointing_session", project_pointing_session);
        },
        get_project_pointing_session: function() {
            return this.load("project_pointing_session", {});
        },
